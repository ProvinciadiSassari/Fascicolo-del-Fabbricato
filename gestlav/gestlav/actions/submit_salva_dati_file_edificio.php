...
if ($tipo_operazione=="mod") {
    ...
    if (!$result){
            die ($query."Could not query the database: mod<br />". mysql_error());
    }   
}
    ...
    $query="INSERT INTO descrizione_files_edifici (
          ...
          VALUES
            (
              ".$_POST["id_fabbricato"].", ".conv_string2sql($dir).", ".conv_string2sql($_POST["file_solo"]).", ".conv_string2sql($_POST["ta_descrizione_file_dett"]).", '".$_SESSION["username"]."', ".conv_string2sql($_POST["ta_catalogazione_file_dett"]).", '".$utility->convertDateToDB($_POST["data_scadenza_documento"])."', ".$_POST["fl_rinnovato"].", now()
            )";
